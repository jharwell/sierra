...
def test_rdrw():
    ...
    exp1 = ExpDef(input_fpath="./tests/unit_tests/expdef/f1.xml", write_config=config)

    exp1.write("/tmp/f1-out.xml")

    diff = main.diff_files("./tests/unit_tests/expdef/f1.xml", "/tmp/f1-out.xml")

    assert len(diff) == 0


def test_attr():
    exp1 = ExpDef(input_fpath="./tests/unit_tests/expdef/f1.xml")
...
def test_element():
    ...
    assert not exp1.element_change(
        ".//controllers", "crw_controller", "foobar_controller"
    )


def test_write_root_rename():
    config = definition.WriterConfig(
        [{"rename_to": "foobar", "src_parent": None, "src_tag": "."}]
    )
    f = ExpDef(input_fpath="./tests/unit_tests/expdef/f2.xml", write_config=config)
...
def test_write_subtree():
    config = definition.WriterConfig([{"src_parent": ".", "src_tag": "framework"}])
    f = ExpDef(input_fpath="./tests/unit_tests/expdef/f2.xml", write_config=config)
    assert f.tree.getroot().tag == "argos-configuration"
...
def test_write_add_grafts():
    ...
    f = ExpDef(input_fpath="./tests/unit_tests/expdef/f3.xml", write_config=config)
