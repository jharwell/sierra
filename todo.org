* SIERRA                                                             :sierra:
** TODO [#B] Add additional integration tests [90%]
...
- [X] Simple model for testing in the sample project, associated tests
- [X] Switch from bash to something else (python/noxfile?); my scripts are
...
** DONE Added tutorial for creating a new model
- And add an example in the sample project
** DONE Update other plugin tutorials {storage, expdef}
After the {platform, execenv} docs overhaul, these tutorials look really bad by
comparison.
** DONE Add additional unit tests [100%]
- [X] Parser unit tests (for CLI variables that can be used as batch criteria)
- [X] Nodefile parsing
