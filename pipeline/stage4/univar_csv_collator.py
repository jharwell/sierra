...
class UnivarCSVCollator:
    ...
    univar batch criteria. Each collated .csv file will have one column per experiment, named after
    the experiment directory, containing a column drawn a .csv in the experiment's output's, per
    graph configuration. Each column containing simulation samples over time.
    ...
    def __collate_target_files(self, target):
        ...
        and collate files, assuming unary batch criteria was used. 
        """

        data_df_new = pd.DataFrame(columns=self.batch_criteria.gen_exp_dirnames(self.cmdopts))
        stddev_df_new = pd.DataFrame(columns=self.batch_criteria.gen_exp_dirnames(self.cmdopts))
