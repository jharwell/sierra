...
class CSV:
    def __init__(self, arg, delimiter=None):
        ...
        arg: Either a CSV object (in which case the init function makes a duplicate) or a file path
             that the CSV object should load its data from
        delimiter: A string of the symbol that is used to separate the symbols inside the file. This
                   can be kept as None if arg is not a file path.
        '''
        if isinstance(arg, CSV):
            ...
            if isinstance(arg, list):
                ...
                raise TypeError(
                    "Incorrect parameter type '{}' for CSV class initialization: must be list or string".format(type(arg)))
    ...
    def __eq__(self, other):
        ...
        if isinstance(other, CSV):
            ...
            raise NotImplementedError(
                "Equality operator between CSV object and object of type '{}' is not supported".format(type(other)))
    ...
    def __add__(self, other):
        ...
        if isinstance(other, CSV):
            ...
            if dims != other.dims:
                raise ValueError(
                    "CSVs with dimensions {} and {} cannot be added together".format(self.dims, other.dims))
            height, width = dims
            ans = []
            ...
            raise NotImplementedError(
                "Addition between CVS objects and objects of type '{}' is not supported".format(type(other)))
    ...
    def __truediv__(self, other):
        ...
        if isinstance(other, numbers.Number):
            ...
            raise NotImplementedError(
                "Division of a CVS object by an object of type '{}' is not supported".format(type(other)))
    ...
    def _add_values(self, val1, val2):
        ...
        try:
            ...
            if val1 != val2:
                raise ValueError(
                    "Cannot add together the strings '{}' and '{}' because they are different strings".format(val1, val2))
            return val1
