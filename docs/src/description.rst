...
- Maximizing the reproducibility of scientific research, particular in AI; it is
  designed so that *no* copy-pasting of code/configuration btween projects is
  needed.

It supports a wide range of execution engines /environments, and experiment
input/output formats via :ref:`plugins <plugins>`. SIERRA supports mix-and-match
between all plugin types, subject to restrictions within the plugins
themselves. This is THE most powerful feature of SIERRA, and makes it very easy
to run experiments on different hardware, targeting different simulators,
generating different outputs, etc., all with little to no configuration changes
by the user.
