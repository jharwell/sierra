...
- Those imagized from project :term:`Raw Output` files via ``--project-imagizing`` using
  ``--project-rendering``. See :ref:`here <plugins/proc/imagize>` for details
  about the project-based imagizing plugin.
...
.. _plugins/deliverable/render/engine:
...
.. _plugins/deliverable/render/project:

Project Rendering
=================

If a project has generated images using :ref:`<plugins/proc/imagize>` in stage 3
via ``--project-imagizing``, then they can be rendered into videos in stage 4.
...
.. _plugins/deliverable/render/bc:
