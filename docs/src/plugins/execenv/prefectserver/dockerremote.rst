...
- The specified :ref:`--docker-image<src/reference/cli:sierra-cli---docker-image>` is
  accessible. It doesn't have to be local; if will be fetched IFF it isn't found
  locally.

- The specified :ref:`--docker-image<src/reference/cli:sierra-cli---docker-image>` contains the
  ``prefect`` executable (this is checked). The version of the prefect
  executable installed should match the version which SIERRA uses; if this isn't
  the case, you may get weird API-based failures.
...
.. IMPORTANT:: If not using rootless docker, files within the container for all
               ...
               :ref:`--docker-is-host-user<src/reference/cli:sierra-cli---docker-is-host-user>` is
               passed.
