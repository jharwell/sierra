...
class ExecEnvChecker():
    ...
    def __call__(self) -> None:
        # Verify we can contact all of the robots
        with open(self.cmdopts['nodefile'], 'r') as f:
            lines = f.readlines()

            for line in lines:
                cores, ssh = line.split('/')
                if int(cores) != 1:
                    self.logger.warning(("Nodefile % s, line '%s' has multiple "
                                         "cores; turtlebots are single core"),
                                        self.cmdopts['nodefile'],
                                        line)
                if '@' in ssh:
                    _, hostname = ssh.split('@')
                else:
                    hostname = ssh
                ...
                try:
                    ...
                    self.logger.fatal("Unable to contact '%s'", hostname)
                    raise
