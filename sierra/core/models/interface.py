...
class IConcreteIntraExpModel1D(implements.Interface):
    ...
    def run_for_exp(
        self, criteria: bc.IConcreteBatchCriteria, cmdopts: types.Cmdopts, exp_num: int
    ) -> bool:
        """
...
class IConcreteIntraExpModel2D(implements.Interface):
    ...
    :term:`Experiment`. Models "target" one or more graphs which are already
...
class IConcreteInterExpModel1D(implements.Interface):
    ...
    :func:`~sierra.core.graphs.summary_line`.  Models "target" one or more CSV files
    which are already configured to be generated, and will show up as additional
    lines on the generated graph.
    """
    ...
    def run_for_batch(
        self, criteria: bc.IConcreteBatchCriteria, cmdopts: types.Cmdopts
    ) -> bool:
        """
