...
def parse(arg: str, dflts: types.SimpleDict) -> types.CLIArgSpec:
    ...
    assert res is not None, f"Bad duration spec in section '{sections[0]}'"
    ...
    if len(sections) >= 2:
        ...
        assert res is not None, f"Bad ticks per second spec in section '{sections[1]}'"
        ret["n_ticks_per_sec"] = int(res.group(0)[1:])

    # Parse # datapoints to capture, which can be absent
    if len(sections) == 3:
        res = re.search(r"N\d+", sections[2])
        assert res is not None, f"Bad # datapoints spec in section '{sections[2]}'"
