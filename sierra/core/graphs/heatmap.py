...
def generate_confusion(  # noqa: PLR0913,PLR0917
    ...
    *,
    ...
    required_cols = [truthcol, predcol]
    ...
    confusion_df = df.group_by([truthcol, predcol]).agg(pl.len().alias("count"))

    # Get category names. Need union in case the sets aren't the same.
    categories = sorted(
        set(df[truthcol].unique().to_list()) | set(df[predcol].unique().to_list())
    )

    # Create all combinations of categories
    all_combinations = pl.DataFrame(
        {
            truthcol: [t for t in categories for _ in categories],
            predcol: [p for _ in categories for p in categories],
        }
    )

    # Merge with actual data, filling missing combinations with 0
    confusion_df = all_combinations.join(
        confusion_df, on=[truthcol, predcol], how="left"
    )
    ...
    row_totals = confusion_df.group_by(truthcol).agg(
        pl.col("count").sum().alias("row_total")
    )

    # Join back and calculate fractions
    confusion_df = confusion_df.join(row_totals, on=truthcol)
    ...
    dataset = hv.Dataset(confusion_pd, kdims=[predcol, truthcol], vdims="fraction")
...
def generate_numeric(  # noqa: PLR0913,PLR0917
    pathset: _pathset.PathSet,
    input_stem: str,
    output_stem: str,
    medium: str,
    ...
    *,
    ...
    if not all(c in df.columns for c in [xcol, ycol, zcol]):
        _logger.warning(
            ...
            output_fpath.relative_to(pathset.batchroot),
            [xcol, ycol, zcol],
            set({xcol, ycol, zcol}) - set(df.columns),
        )
        return False
    ...
    kdims = [ycol, xcol] if transpose else [xcol, ycol]

    dataset = hv.Dataset(df_pd, kdims=kdims, vdims=zcol)
    ...
    plot = hv.HeatMap(dataset, kdims=[xcol, ycol], vdims=[zcol])
    if backend == "matplotlib":
        plot.opts(show_values=False)

    if not xticks:
        xticks = dataset.data[xcol]
    if not yticks:
        yticks = dataset.data[ycol]
    # Add X,Y ticks
    if xticklabels:
