...
class CameraTimeline(IBaseVariable):
    """
    ...
    def gen_tag_rmlist(self):
        ...
        """
        return [set([("./visualization/qt-opengl", "camera")])]

    def gen_tag_addlist(self):
        adds = [('./visualization/qt-opengl', 'camera', {}),
                ("./visualization/qt-opengl/camera", "placements", {})]  # type: tp.List[tuple]

        in_ticks = self.tsetup.sim_duration * time_setup.kTICKS_PER_SECOND
        adds.append(('.//qt-opengl/camera', 'timeline', {'loop': str(in_ticks)}))

        for c in range(0, self.kARGOS_N_CAMERAS):
            adds.append(('.//qt-opengl/camera/timeline',
                         'keyframe',
                         {
                             'placement': str(c),
            ...
            if c < self.kARGOS_N_CAMERAS - 1:
                adds.append(('.//qt-opengl/camera/timeline', 'interpolate', {}))
        return [adds]
