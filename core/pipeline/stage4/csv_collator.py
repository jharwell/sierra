...
class UnivarCSVCollator:
    ...
    experiments for univariate batch criteria. Each collated ``.csv`` file will have one column per
    experiment, named after the experiment directory, containing a column drawn from a ``.csv`` in
    the experiment's averaged output, per graph configuration.
    ...
    def __call__(self, batch_criteria, targets: dict):
        ...
        for category in targets:
            ...
            for graph in category['graphs']:
                self.__collate_target_files(graph, batch_criteria)

    def __collate_target_files(self, target, batch_criteria):
        """
        For a single graph (target), go through all experiment directories in a batched experiment
        and collate files, assuming univariate batch criteria was used.
        """
...
class BivarCSVCollator:
    """
    Generates collated ``.csv`` files from the averaged ``.csv`` files present in a batch of
    experiments for bivariate batch criteria. Each collated ``.csv`` file will one criteria as the
    column values, and one criteria as the row index, with each (x,y) pair containing a vector of
    temporal experiment values drawn from a ``.csv`` in the experiment's averaged output, per graph
    configuration.
    ...
    def __collate_exp_csv_stddev(self, exp_dir: str, target, collated_df):
        exp_output_root = os.path.join(self.batch_output_root, exp_dir)
        stddev_ipath = os.path.join(exp_output_root,
                                    self.main_config['sierra']['avg_output_leaf'],
                                    target['src_stem'] + '.stddev')
