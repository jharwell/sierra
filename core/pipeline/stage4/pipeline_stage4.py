...
class PipelineStage4:
    ...
    def _load_models(self):
        ...
        self.logger.info("Stage4: Loading models for project '%s'", self.cmdopts['project'])
        ...
        if len(self.models_intra) > 0:
            self.logger.info("Stage4: Loaded %s intra-experiment models for project '%s'",
                             len(self.models_intra),
                             self.cmdopts['project'])

        if len(self.models_inter) > 0:
            self.logger.info("Stage4: Loaded %s inter-experiment models for project '%s'",
    ...
    def _load_LN_config(self):
        ...
        if core.utils.path_exists(project_intra_LN):
            self.logger.info("Stage4: Loading additional intra-experiment linegraph config for project '%s'",
                             self.cmdopts['project'])
        ...
        if core.utils.path_exists(project_inter_LN):
            self.logger.info("Stage4: Loading additional inter-experiment linegraph config for project '%s'",
                             self.cmdopts['project'])
    ...
    def _load_HM_config(self):
        ...
        if core.utils.path_exists(project_intra_HM):
            self.logger.info("Stage4: Loading additional intra-experiment heatmap config for project '%s'",
                             self.cmdopts['project'])
    ...
    def _run_rendering(self):
        ...
        self.logger.info("Stage4: Rendering videos...")
        start = time.time()
        BatchedExpVideoRenderer()(self.main_config, render_opts, self.cmdopts['batch_output_root'])
        elapsed = int(time.time() - start)
        sec = datetime.timedelta(seconds=elapsed)
        self.logger.info("Stage4: Rendering complete in %s", str(sec))

    def _run_intra_models(self, batch_criteria):
        self.logger.info("Stage4: Running intra-experiment models...")
        start = time.time()
        ...
        elapsed = int(time.time() - start)
        sec = datetime.timedelta(seconds=elapsed)
        self.logger.info("Stage4: Intra-experiment models finished in %s", str(sec))

    def _run_inter_models(self, batch_criteria):
        self.logger.info("Stage4: Running inter-experiment models...")
        start = time.time()
        ...
        elapsed = int(time.time() - start)
        sec = datetime.timedelta(seconds=elapsed)
        self.logger.info("Stage4: inter-experiment models finished in %s", str(sec))

    def _run_intra_graph_generation(self, batch_criteria):
        """
        Generate intra-experiment graphs (duh).
        """
        self.logger.info("Stage4: Generating intra-experiment graphs...")
        start = time.time()
        ...
        elapsed = int(time.time() - start)
        sec = datetime.timedelta(seconds=elapsed)
        self.logger.info("Stage4: Intra-experiment graph generation complete: %s", str(sec))

    def _run_collation(self, batch_criteria):
        targets = self._calc_inter_LN_targets()

        if not self.cmdopts['no_collate']:
            self.logger.info("Stage4: Collating inter-experiment .csv files...")
            start = time.time()
            MultithreadCollator(self.main_config, self.cmdopts)(batch_criteria, targets)
            elapsed = int(time.time() - start)
            sec = datetime.timedelta(seconds=elapsed)
            self.logger.info("Stage4: Collating inter-experiment .csv files complete: %s", str(sec))
    ...
    def _run_inter_graph_generation(self, batch_criteria):
        ...
        self.logger.info("Stage4: Generating inter-experiment graphs...")
        start = time.time()
        InterExpGraphGenerator(self.main_config, self.cmdopts, targets)(batch_criteria)
        elapsed = int(time.time() - start)
        sec = datetime.timedelta(seconds=elapsed)
        self.logger.info("Stage4: Inter-experiment graph generation complete: %s", str(sec))
