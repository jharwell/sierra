...
def _conf95_kernel(df_like, groupby: bool) -> tp.Dict[str, pd.DataFrame]:
    ...
    return {
        core.config.kStatsExtensions['mean']: df_like.mean().round(8),
        core.config.kStatsExtensions['stddev']: df_like.std().round(8)
    }
...
def _bw_kernel(df_like, groupby: bool, n_sims: int) -> tp.Dict[str, pd.DataFrame]:
    ...
    return {
        core.config.kStatsExtensions['mean']: csv_mean,
        core.config.kStatsExtensions['median']: csv_median,
        core.config.kStatsExtensions['q1']: csv_q1,
        core.config.kStatsExtensions['q3']: csv_q3,
        core.config.kStatsExtensions['cilo']: csv_cilo,
        core.config.kStatsExtensions['cihi']: csv_cihi,
        core.config.kStatsExtensions['whislo']: csv_whislo,
        core.config.kStatsExtensions['whishi']: csv_whishi
    }
